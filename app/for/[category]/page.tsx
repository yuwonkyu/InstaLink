...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const title = meta.headline;
  const description = meta.subheadline;
...
export default async function CategoryLandingPage({ params }: PageProps) {
  ...
  const jsonLd = {
    ...
    offers: {
      ...
      price: "0",
      priceCurrency: "KRW",
      description: "무료로 시작, Basic 29,000원/월, Pro 49,000원/월",
    },
  };
      ...
      <header className="sticky top-0 z-20 border-b border-black/5 bg-white/90 backdrop-blur">
        <div className="mx-auto flex w-full max-w-3xl items-center justify-between px-4 py-3 sm:px-6">
          ...
          <nav className="flex items-center gap-2 text-sm font-medium">
            <Link href="/auth/login" className="rounded-lg px-3 py-1.5 text-(--muted) hover:text-foreground">
              로그인
            </Link>
            <Link
              href="/auth/signup"
              className="rounded-lg bg-foreground px-3 py-1.5 text-white hover:opacity-80 transition-opacity"
            >
              무료 시작
            </Link>
          </nav>
      ...
      <div className="mx-auto w-full max-w-3xl px-4 py-10 sm:px-6">
        ...
        <section className="rounded-3xl bg-(--card) px-6 py-10 shadow-[0_4px_20px_rgba(17,24,39,0.06)] sm:px-10 sm:py-14">
          ...
          <div className="mt-8 flex flex-wrap gap-3">
            <Link
              href="/auth/signup"
              className="rounded-xl bg-foreground px-5 py-2.5 text-sm font-semibold text-white hover:opacity-80 transition-opacity"
            >
              무료로 시작하기
            </Link>
            {meta.sampleSlug && (
              <Link
                ...
                className="rounded-xl border border-black/10 bg-white px-5 py-2.5 text-sm font-semibold hover:bg-black/5 transition-colors"
              >
                예시 보기
              </Link>
            )}
        ...
        <section className="mt-6 rounded-2xl bg-(--card) px-6 py-6 shadow-[0_4px_20px_rgba(17,24,39,0.06)]">
          ...
          <div className="mt-4 grid gap-3 sm:grid-cols-3">
            {[
              { name: "Free", price: "무료", desc: "기본 프로필, 서비스 3개" },
              { name: "Basic", price: "29,000원/월", desc: "서비스 무제한, 모든 테마, 후기 무제한" },
              { name: "Pro", price: "49,000원/월", desc: "Basic + 방문자 통계, 우선 지원" },
            ].map((p) => (
              <div key={p.name} className="rounded-xl bg-(--secondary) p-4">
        ...
        <section className="mt-6 rounded-2xl bg-foreground px-6 py-8 text-center text-white shadow-[0_4px_20px_rgba(17,24,39,0.12)]">
          ...
          <p className="mt-2 text-sm opacity-75">
            신용카드 불필요 · 5분 설정 · 언제든지 취소
          </p>
          <Link
            href="/auth/signup"
            className="mt-5 inline-block rounded-xl bg-white px-6 py-3 text-sm font-semibold text-foreground hover:bg-white/90 transition-colors"
          >
            무료로 시작하기 →
          </Link>
        </section>
            ...
            {Object.entries(CATEGORIES)
              ...
              .map(([key, c]) => (
                <Link
                  key={key}
                  href={`/for/${key}`}
                  className="rounded-full border border-black/10 bg-white px-3 py-1.5 text-xs font-medium text-foreground hover:bg-(--secondary) transition-colors"
                >
                  {c.label}
                </Link>
              ))}
